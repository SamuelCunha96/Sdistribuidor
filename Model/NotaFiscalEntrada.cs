    ...
    public class NotaFiscalEntrada
    {
        ...
        public DataTable ConsultaNFeSemEntrada(DateTime DtAutorizacaoIni, DateTime DtAutorizacaoFim)
        {
            return BancoDados.Consultar(" SELECT itc.nomeemitente,SUBSTR(itc.chacessonfe,23,3) as serienf,SUBSTR(itc.chacessonfe,26,9)::integer as nrnf,itc.chacessonfe,itc.vlnfe " +
                                        " FROM downloadnfe d " +
                                        " INNER JOIN itemconsultanfdest itc on d.txchacessonfe = itc.chacessonfe " +
                                        " WHERE not exists(select * from EntNotaFiscal nf where d.txchacessonfe = nf.txchacessonfe) and xmlnfe is not null" +
        ...
        public bool IncluirNFEntrada(Entidade_NotaFiscal NfEntrada)
        {
            ...
            try
            {
                ...
                command = new NpgsqlCommand("itemnfentrada", BancoDados.conexao);
                ...
                command.Parameters.AddWithValue("dtentrada", NfEntrada.dtentrada);
                command.Parameters.AddWithValue("vltotal", NfEntrada.vltotal);
                command.Parameters.AddWithValue("vlbaseicms", NfEntrada.vlbaseicms);
                command.Parameters.AddWithValue("vlicms ", NfEntrada.vlicms);
                command.Parameters.AddWithValue("vlbaseicmssub", NfEntrada.vlbaseicmssub);
                command.Parameters.AddWithValue("vlicmssub", NfEntrada.vlicmssub);
                command.Parameters.AddWithValue("vloutras", NfEntrada.vloutras);
                command.Parameters.AddWithValue("vltributos", NfEntrada.vltributos);
                command.Parameters.AddWithValue("vlfrete", NfEntrada.vlfrete);
                command.Parameters.AddWithValue("vlseguro", NfEntrada.vlseguro);
                ...
                foreach (var item in NfEntrada.ItemNFe)
                {
                    ...
                    command.Parameters.AddWithValue("qt_entrada", Convert.ToDecimal(item.qt_venda));
                    ...
                    command.Parameters.AddWithValue("vlseguro", item.vloutras);
